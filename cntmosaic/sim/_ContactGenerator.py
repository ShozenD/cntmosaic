...
class ContactGenerator:
	...
	**Example 3: Full case (All subgroup interactions)**
	...
	def __init__(
		...
		cint_matrices: Union[NDArray, Dict[Union[int, str], NDArray], Dict[Tuple, NDArray]],
		model: str = 'poisson',
		odisp: float = None,
	...
	def _validate_matrices(self) -> None:
		...
		if isinstance(self.cint_matrices, dict):
			...
			if self.is_full_case:
				...
				matrix_labels = set(self.cint_matrices.keys())
	...
	def generate(self, seed: int = None) -> pd.DataFrame:
		...
		for source_label in self.subgroup_labels:
			...
			df_part_source = self.df_part[self.df_part['subgroup'] == source_label].copy()
	...
	def summarize_contacts(self, contacts: pd.DataFrame = None) -> pd.DataFrame:
		...
		if self.is_full_case:
			...
			for source_sg in self.subgroup_labels:
				for target_sg in self.subgroup_labels:
					...
					source_participants = self.df_part[self.df_part['subgroup'] == source_sg]['id']
					contacts_source_to_target = contacts_filtered[contacts_filtered['id'].isin(source_participants)]
					
					if len(contacts_source_to_target) > 0:
						total_contacts = contacts_source_to_target['y'].sum()
						n_participants = len(source_participants)
						mean_contacts = total_contacts / n_participants if n_participants > 0 else 0
					else:
						total_contacts = 0
						n_participants = len(source_participants)
						mean_contacts = 0
		...
		summary = participant_totals.groupby('subgroup')['total_contacts'].agg([
		...
		if self.is_full_case:
			...
			for source_label in self.subgroup_labels:
				for target_label in self.subgroup_labels:
					...
					df_part_source = self.df_part[self.df_part['subgroup'] == source_label]
					...
					# Normalize if requested
					if normalize:
						age_counts = df_part_source['age_group'].value_counts().sort_index()
						for i in range(n_age_groups):
							if i in age_counts.index and age_counts[i] > 0:
								empirical[i, :] /= age_counts[i]

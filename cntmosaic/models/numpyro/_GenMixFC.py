"""NumPyro model mixin for GenMixFC."""
...
class GenMixFCNumPyroMixin:
    """Carries ``sample_log_delta()`` and ``model()`` for GenMixFC."""
    ...
    def model(
        ...
        log_cint_tensor = (
            log_rate[jnp.newaxis, :, :] + self.log_P[:, jnp.newaxis, :] + log_delta
        )

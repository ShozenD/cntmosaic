...
def test_basic_2d_case():
  ...
  prec = np.array([[2.0, 0.5], 
                   [0.5, 1.0]])
  factor = sksp.cholmod.cholesky(sp.csc_matrix(prec))
  # Condition on x[1], compute conditional for x[0]
  ix_target = np.array([0])
  ix_cond = np.array([1])

  cond_mean, cond_factor = mvn_cond_params(x, mean, factor, ix_target, ix_cond)
  ...
  np.testing.assert_allclose(cond_factor, expected_prec, rtol=1e-10)

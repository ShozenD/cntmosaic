...
def plot_mosaic(
    ...
    # Plot the matrix
    im = ax.imshow(cint.T, cmap=cmap, origin='lower', vmin=vmin, vmax=vmax)
    
    # Set axis labels and title
    ax.set_title(title, fontsize=9, loc='left') if title else None
    ax.set_xlabel(xlabel, fontsize=8) if xlabel else ax.set_xticklabels([])
    ax.set_ylabel(ylabel, fontsize=8) if ylabel else ax.set_yticklabels([])

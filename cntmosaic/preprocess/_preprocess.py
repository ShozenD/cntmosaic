...
def impute_age_min_max(
    ...
    if dropna:
        ...
        n1 = data.shape[0]
        warnings.warn(
            f"Dropped {n0 - n1} rows with missing values in {age_col}", UserWarning
        )
...
def make_train_data(
    data: pd.DataFrame, id_var: str, strat_vars: str | list[str] | None = None
) -> pd.DataFrame:
    """
    ...
    strat_vars : str or list[str], optional
    ...
    if "y" not in data.columns:
        warnings.warn(
            'No column "y" found. Assuming each row represents a single contact.',
            RuntimeWarning,
        )
        data["y"] = 1
    ...
    non_numeric_cols = data[strat_vars_cnt].select_dtypes(include="object").columns
    ...
    # ===== Calculate N and y =====
    df_N = (
        data.groupby(strat_vars_part, observed=False)
        .agg(N=(id_var, "nunique"))
        .reset_index()
    )
    df_y = (
        data.groupby(strat_vars_cnt, observed=False).agg(y=("y", "sum")).reset_index()
    )

    # ===== Create the grid =====
    df_grid = make_full_grid(data, age_vars, strat_vars)

    # ===== Merge the dataframes =====
    df = pd.merge(df_grid, df_y, on=strat_vars_cnt, how="left")
    df = pd.merge(df, df_N, on=strat_vars_part, how="left")
...
def add_grp_cnt_offsets(
    df_cnt: pd.DataFrame,
    df_grp: pd.DataFrame,
    strat_vars: str | list[str] | None = None,
    max: int | None = None,
) -> pd.DataFrame:
    """Create offsets for group contacts and contacts with missing information.

    Suppose strat_vars = ['sex_part']. Given the total number of contacts with complete information :math:`Y^g_a = sum_{b} Y^{g}_{a,b}`
    ...
    df_grp : pd.DataFrame
        ...
        in a column named `z`.
    strat_vars : str | list | None, optional
    ...
    if "z" not in df_grp.columns:
        raise RuntimeError(
            'No column "z" found. Please provide the number of group contacts in a column named "z"'
        )
    ...
    df_offsets = pd.merge(df_cnt_part, df_grp_sum, on=strat_vars, how="left")
    df_offsets["S"] = df_offsets["y"] / (df_offsets["y"] + df_offsets["z"])
    df_offsets["S"] = df_offsets["S"].fillna(0)
    df_offsets["S"] = np.where(
        df_offsets["S"] == 0, 1, df_offsets["S"]
    )  # Avoid log(0) = -inf errors

    cols = strat_vars + ["S"]
    return pd.merge(df_cnt, df_offsets[cols], on=strat_vars, how="left")

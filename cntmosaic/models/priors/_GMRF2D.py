...
class GMRF2D(Prior2D):
    ...
    def _set_grid(self):
        ...
        if self.type == "global":
            self.L, self.sym_idx = gmrf2d_sym_operators(
                self.A, self.order, cov_struct="additive"
            )
        elif self.type == "partial":
            self.L = gmrf2d_operators(
                (self.A, self.A), self.order, cov_struct="additive"
            )
        elif self.type == "full":
            self.L_diag, self.sym_idx = gmrf2d_sym_operators(
                (self.A, self.A), self.order, cov_struct="additive"
            )
            self.L_non_diag = gmrf2d_operators(
                self.A, self.order, cov_struct="additive"
            )
    ...
    def sample(self):
        ...
        if self.type == "global":
            ...
            def map_gmrf(x):
                return gmrf(x, self.L, self.scale)

            plate_event = numpyro.plate("event", self.event_dim_eff, dim=-2)
            with plate_event:
                z = numpyro.sample("z", dist.Normal(0, 1), sample_shape=(self.A**2,))
            f = vmap(map_gmrf)(z)
            f = self.trans_loc + f.reshape(
                (self.event_dim_eff, self.A, self.A), order="F"
            )

        elif self.type == "full":

            def map_gmrf_sym(x):
                return gmrf_sym(x, self.L_diag, self.sym_idx, self.scale)

            def map_gmrf(x):
                return gmrf(x, self.L_non_diag, self.scale)
            ...
            with plate_diag:
                ...
                z_diag = numpyro.sample(
                    "z_diag", dist.Normal(0, 1).expand((N,)).to_event(1)
                )

            with plate_non_diag:
                z_non_diag = numpyro.sample(
                    "z_non_diag", dist.Normal(0, 1).expand((self.A**2,)).to_event(1)
                )
            ...
            raise ValueError(
                f"Unknown type '{self.type}'. Must be one of 'global', 'partial', or 'full'."
            )

...
def merge_zero_groups(intervals, counts):
    ...
    assert (
        sum(counts) > 0
    ), "All counts are zero. There are no participants in any group."
...
class SocialMix:
    ...
    def check_age_bins(self):
        ...
        ssizes = (
            tmp.groupby("age_grp_part", observed=False).size().reset_index(name="N")
        )
        if np.min(ssizes["N"]) == 0:
            if self.adaptive_merge:
                ...
                ValueError(
                    "Some age groups have zero sample sizes. Set adaptive_merge=True to merge age groups adaptively."
                )

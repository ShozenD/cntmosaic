...
class BaseLoader(ABC):
    def __init__(self,
        ...
        # maybe move this to documentation page, provide a link
            ...
            sample = pd.DataFrame({'age':[1,2,1,2],
                                   'category':['A', 'A', 'B', 'B'], 
    ...
    def _load_pop_proportions(self):
        ...
        for q in self.pop_prop:
            ...
            pivoted = df.sort_values(var_name).pivot(columns='age', index=var_name, values=prop_name)
    ...
    def load(self) -> xr.Dataset:
        ...
        df_n = self.data.groupby(grp_vars_n, observed=False).agg(
            N=(self.col_map.id_var, 'nunique')).reset_index()

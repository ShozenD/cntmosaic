...
class HiBRCfine(BRCfine):
    ...
    def __init__(
        self, dataloader: DataLoader, priors: dict[Prior2D], likelihood: str = "negbin"
    ):
        ...
        self.X_ids = {
            c: pd.Categorical(
                self.ds[c].values, categories=sorted(set(self.ds[c].values))
            ).codes
    ...
    def model(self, y=None):
        ...
        if self.likelihood == "negbin":
            ...
            with plate("data", len(self.y)):
                numpyro.sample(
                    "obs",
                    dist.NegativeBinomial2(mean=mu, concentration=1 / inv_disp),
                    obs=y,
                )

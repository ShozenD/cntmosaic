...
class vdKassteele(Prior2D):
    ...
    def __init__(self, transform: str | None = None, prior_type: str = "global"):
    ...
    def _set_grid(self):
        ...
        if self.type == "global":
            self.L, self.sym_idx = gmrf2d_sym_operators(
                self.A, (2, 2), cov_struct="additive"
            )
            ...
            self.L_diag, self.sym_idx = gmrf2d_sym_operators(
                (self.A, self.A), (2, 2), cov_struct="additive"
            )
    ...
    def sample(self):
        ...
        if self.type == "global":
            ...
            def map_gmrf_sym(x):
                return gmrf_sym(x, self.L_diag, self.sym_idx, tau)

            def map_gmrf(x):
                return gmrf(x, self.L_non_diag, tau)

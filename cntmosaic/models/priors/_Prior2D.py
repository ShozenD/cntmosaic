...
class Prior2D(ABC):
  ...
  def set_loc(self, loc: int | float | np.ndarray):
    ...
    if isinstance(loc, (int, float)): # If loc is a scalar
      ...
      return  # Early return for scalar case
    
    valid_shapes = {
      (self.event_dim_eff, self.A, self.A): lambda x: x,
      (self.event_dim_eff, self.A): lambda x: jnp.repeat(x[:, :, None], self.A, axis=2)
    }
    ...
    if data is None:
      warnings.warn(
        ...
        f"({self.event_dim_eff}, {self.A}, {self.A}) or "
        f"({self.event_dim_eff}, {self.A}) but got {loc.shape}. "
        "Please check the input shape or manually set the age bounds."
      )
      # Set a default value to avoid leaving trans_loc undefined
      self.trans_loc = jnp.full((self.event_dim_eff, self.A, self.A), 0.0)
    elif self.transform:

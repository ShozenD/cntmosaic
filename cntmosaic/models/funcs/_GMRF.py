...
def gmrf1d_operators(order, num_nodes, eps=1e-6):
...
def gmrf2d_operators(num_nodes, order, cov_struct="additive", eps=1e-6):
...
def gmrf2d_sym_operators(num_nodes, order, cov_struct="additive", eps=1e-6):
...
@jit
def gmrf(x, L, scale=1.0):
	return scale * L @ x

@jit
def gmrf_sym(x, L, sym_idx, scale=1.0):
	return scale * (L @ x)[sym_idx]

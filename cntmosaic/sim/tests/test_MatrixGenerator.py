...
def test_template_normalization():
  ...
  for name, template in generator.templates.items():
    ...
    assert np.isclose(mean_intensity, 1.0), f"Template {name} not normalized correctly"
...
def test_generate_partial_basic():
  ...
  assert 0 in matrices, "Should have matrix for subgroup 0"
  assert 1 in matrices, "Should have matrix for subgroup 1"
...
def test_generate_partial_reciprocity():
  ...
  for label, M in matrices.items():
    P = np.diag(subgroups[label].age_dist)
    PM = P @ M
...
def test_generate_partial_reproducibility():
  ...
  for label in matrices1.keys():
    assert np.allclose(matrices1[label], matrices2[label]), f"Matrices for subgroup {label} should be identical"
...
def test_generate_partial_single_subgroup():
  ...
  assert 0 in matrices, "Should have matrix for subgroup 0"
  assert matrices[0].shape == (4, 4), "Matrix should be 4x4"
...
def test_generate_full_within_subgroup_symmetry():
  ...
  for i in range(len(subgroups)):
    ...
    PM_ii = P_i @ M_ii
    assert np.allclose(PM_ii, PM_ii.T), f"Within-subgroup matrix ({i},{i}) should satisfy PM = PM^T"
...
def test_generate_full_between_subgroup_reciprocity():
  ...
  assert np.allclose(P_0 @ M_01, (P_1 @ M_10).T), "Between-subgroup reciprocity not satisfied"
...
def test_generate_full_three_subgroups():
  ...
  for i in range(3):
    for j in range(3):
      assert (i, j) in matrices, f"Should have matrix for ({i}, {j})"
...
def test_generate_full_reproducibility():
  ...
  for key in matrices1.keys():
    assert np.allclose(matrices1[key], matrices2[key]), f"Matrices for {key} should be identical"
...
def test_get_contact_rate_matrix():
  ...
  assert np.allclose(Gamma, expected_Gamma), "Contact rate matrix calculation incorrect"
...
def test_get_contact_rate_matrix_reciprocity():
  ...
  assert np.allclose(M, M_reconstructed), "Should be able to reconstruct M from Γ and P"

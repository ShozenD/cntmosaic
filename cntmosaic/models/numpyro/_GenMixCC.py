"""NumPyro model mixin for GenMixCC."""
...
class GenMixCCNumPyroMixin:
    """Carries ``sample_log_delta()`` and ``model()`` for GenMixCC."""
    ...
    def model(
        ...
        # Direct observation-level indexing — no logsumexp needed because both
        # participant and contact ages are already at coarse resolution.
        log_cint = (
            log_rate[cid, did]
            + log_delta[flat_ix, cid, did]
            + self.log_P[flat_pixs, did]
        )

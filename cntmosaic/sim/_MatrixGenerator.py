...
class MatrixGenerator:
	...
	def _validate_templates(self, templates: Dict[str, NDArray]) -> None:
		...
		if first_shape[0] != first_shape[1]:
			raise ValueError(f"Templates must be square matrices. Got shape: {first_shape}")
	...
	def generate_full(
		...
		for i, subgroup_k in enumerate(subgroups):
			for subgroup_l in subgroups[i+1:]:
				...
				mean_intensity = (subgroup_k.mean_cint_margin + subgroup_l.mean_cint_margin) / 2

...
class PSpline2D(Spline2D):
    ...
    def sample(self):
        ...
        if self.prior_type == "global":
            Z = numpyro.sample(
                "spline_coef",
                dist.Normal(0.0, 1.0 / self.order),
            ...
            beta = vmap(self.igmrf_operator, in_axes=(0, 0, 0), out_axes=2)(
                Z, tau, tau * self.tau_ratio
            )  # (M, M, event_dim_eff)
            beta = beta.reshape(
                (self.M * self.M, self.event_dim_eff)
            )  # (M*M, event_dim_eff)
            ...
            beta_diag = vmap(self.igmrf_operator, in_axes=(0, 0, 0), out_axes=2)(
                Z_diag, tau_diag, tau_diag * self.tau_ratio
            )  # shape: (M, M, event_dim_diag)
            beta_diag = beta_diag.reshape(
                (self.M * self.M, self.event_dim_diag)
            )  # shape: (M*M, event_dim_diag)
            ...
            tau_non_diag = jnp.maximum(
                tau_non_diag, 1e-3
            )  # Ensure tau is not too small

            beta_non_diag = vmap(self.igmrf_operator, in_axes=(0, 0, 0), out_axes=2)(
                Z_non_diag, tau_non_diag, tau_non_diag * self.tau_ratio
            )  # shape: (M, M, event_dim_non_diag)
            beta_non_diag = beta_non_diag.reshape(
                (self.M * self.M, self.event_dim_non_diag)
            )  # shape: (M*M, event_dim_non_diag)
            f_non_diag = (self.PHI_non_diag @ beta_non_diag).swapaxes(
                0, 1
            )  # shape: (event_dim_non_diag, A**2)

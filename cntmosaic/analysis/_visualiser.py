...
from ._summariser import ModelSummariserSVI
...
class ModelVisualiser:
  ...
  def __init__(self, summariser: ModelSummariserSVI):
  ...
  def plot_cint(self, width=250, height=250, facet_columns=3, style_config=None):
    ...
    if type(self.summariser.model) in (BRCfine, BRCrefine):
      return alt.Chart(df_from_dict(sum_cint)).mark_rect().encode(
        x=alt.X(
          'x:O',
        ...
        width=width,
        height=height,
        title={'text': ['Estimated contact intensity'], **self.default_config['title']}
      )

...
class IGMRF(Distribution):
  ...
  pytree_data_fields = ("loc", "lam_sub", "conditional_precision", "U", "U_sub", "Q")
  ...
  def __init__(
      ...
      order: int,
      loc: ArrayLike = 0.0,
      conditional_precision: Array = 1.0,
      *,
    ...
    conditional_precision: ArrayLike, optional
    ...
    self.order = order
    self.Q = laplacian(num_nodes, order)
    lam, U = jnp.linalg.eigh(self.Q)
    ...
    if jnp.ndim(conditional_precision) == 0:
      (conditional_precision,) = promote_shapes(conditional_precision, shape=(1,))
    conditional_precision = conditional_precision[..., jnp.newaxis]
      
    loc, U_promoted = promote_shapes(loc, U)
    lam_sub, conditional_precision_promoted = promote_shapes(lam[order:], conditional_precision)
    
    self.U = U_promoted
    self.lam_sub = lam_sub
    self.conditional_precision = conditional_precision_promoted
    self.U_sub = U[:,order:]    # shape (n, n-1)
    self.loc = loc[..., 0]
      
    event_shape = (num_nodes,)
    batch_shape = lax.broadcast_shapes(
      jnp.shape(loc)[:-2],
      jnp.shape(conditional_precision_promoted)[:-1]
    )
  ...
  def sample(
    ...
    scale = scale/self.conditional_precision[..., jnp.newaxis]
    scale = jnp.sqrt(scale)
    
    result = self.loc + jnp.squeeze(
      jnp.matmul(self.U_sub, scale * eps), axis=-1
    )
  ...
  def log_prob(self, value: ArrayLike) -> ArrayLike:
    n = self.num_nodes
    # Ensure value has the correct shape for matrix operations
    if jnp.ndim(value) == 1:
      diff = value - self.loc
      quad = 0.5 * diff.T @ self.Q @ diff
      result = 0.5 * (n * jnp.log(self.conditional_precision[0, 0])
               + jnp.sum(jnp.log(self.lam_sub[0]))) - quad
    else:
      # Handle batched case
      diff = value - self.loc[jnp.newaxis, :]
      quad = 0.5 * jnp.sum((diff @ self.Q) * diff, axis=-1)
      result = 0.5 * (n * jnp.log(self.conditional_precision[0, 0])
               + jnp.sum(jnp.log(self.lam_sub[0]))) - quad
  ...
  @staticmethod
  def infer_shapes(
      num_nodes, order, loc=(), conditional_precision=(),
  ):

...
class ParticipantGenerator:
    ...
    **Example 3: Multiple subgroups with automatic numeric labels**
    ...
    def __init__(self, subgroups: Union[Subgroup, List[Subgroup]]):
        """
    ...
    def _parse_subgroups(self, subgroups: Union[Subgroup, List[Subgroup]]) -> None:
    ...
    def _normalize_distributions(self) -> None:
        ...
        if not self.has_subgroups:
            ...
            self.age_proportions = {
                key: dist / dist.sum() for key, dist in self.age_dists.items()
            }

    @staticmethod
    def _generate_single(
        n: int, age_prop: NDArray, rng: np.random.Generator
    ) -> pd.DataFrame:
        """
